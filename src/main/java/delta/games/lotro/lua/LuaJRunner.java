...
/**
 * Runner for luaJ scripts.
 * 
 * @author DAM
 */
public class LuaJRunner {
  private static Logger LOGGER = Logger.getLogger(LuaJRunner.class);

  private LuaState state;
  private LuaThread mainRoutine;
  private String eventFilter = null;
  
  public LuaJRunner(InputStream script, LuaString name) throws IOException, LuaError, UnwindThrowable {
    ...
    this.state = LuaState.builder()
        ...
        .errorReporter((e, msg) -> {
          //if (LOGGER.isErrorEnabled(Logging.VM_ERROR)) {
            LOGGER.error("Error occurred in the Lua runtime. Computer will continue to execute: " + msg.get());
          //}
        })
        .build();
    ...
    LotroCompanionLua.add(state, globals);
  ...
  public void handleEvent(String eventName, Object[] arguments) throws FileNotFoundException, IOException, CompileException {
    if (eventFilter != null && eventName != null && !eventName.equals(eventFilter) && !eventName.equals("terminate")) {
      //return MachineResult.OK;
    }
    
    try {
      Varargs resumeArgs = eventName == null ? Constants.NONE : ValueFactory.varargsOf(ValueFactory.valueOf(eventName), toValues(arguments));
